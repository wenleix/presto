...
public class RowExpressionCompiler
{
    ...
    public BytecodeNode compile(RowExpression rowExpression, Scope scope, Optional<Variable> outputBlock)
    {
        return compile(rowExpression, scope, outputBlock, Optional.empty());
    }

    // When outputBlock is presented, the generated bytecode will write the evaluated value into the outputBlock
    // otherwise the value will be left on stack.
    public BytecodeNode compile(RowExpression rowExpression, Scope scope, Optional<Variable> outputBlock, Optional<Class> lambdaInterface)
    {
        return rowExpression.accept(new Visitor(), new Context(scope, outputBlock, lambdaInterface));
    }
            ...
            implements RowExpressionVisitor<BytecodeNode, Context>
    {
        ...
        public BytecodeNode visitConstant(ConstantExpression constant, Context context)
        {
            ...
            if (value == null) {
                return block.comment("constant null")
                        ...
                        .append(generateWriteIfNecessary(constant, context));
            }
        ...
        {
            return new BytecodeBlock()
                    .append(fieldReferenceCompiler.visitInputReference(node, context.getScope()))
                    .append(generateWriteIfNecessary(node, context));
        }

        @Override
        public BytecodeNode visitLambda(LambdaDefinitionExpression lambda, Context context)
        {
            checkArgument(!context.getOutputBlock().isPresent(), "lambda definition expression does not support writing to block");
        ...
        {
            checkArgument(!context.getOutputBlock().isPresent(), "variable reference expression does not support writing to block");
            return fieldReferenceCompiler.visitVariableReference(reference, context.getScope());
        }

        private BytecodeNode generateWriteIfNecessary(RowExpression rowExpression, Context context)
        {
    ...
    private static class Context
    {
        ...
        private final Optional<Variable> outputBlock;
        private final Optional<Class> lambdaInterface;

        public Context(Scope scope, Optional<Variable> outputBlock, Optional<Class> lambdaInterface)
        {
            this.scope = scope;
        ...
        public Optional<Variable> getOutputBlock()
        {
            return outputBlock;
        }
